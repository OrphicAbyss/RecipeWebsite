recipe = angular.module('recipe', []).
    config(['$routeProvider', function ($routeProvider) {
            $routeProvider.
                ...
                when('/delete/:recipeId', {templateUrl: 'client/delete.html', controller: DeleteCtrl}).
...
recipe.factory('$recipeServer', function ($http) {
    ...
    return {
        ...
        delete: function (recipeId, successCallback, errorCallback) {
            callServer({cmd: 'delete', id: recipeId}, function (data) {
                if (data.error) {
                    errorCallback(data.message);
                } else {
                    successCallback();
...
function ViewCtrl ($scope, $recipeServer, $routeParams, $location, $dialog) {
    ...
    $scope.delete = function () {
        $location.path("/delete/" + $scope.recipeId);
    };
}
...
function DeleteCtrl ($scope, $recipeServer, $routeParams, $location, $dialog) {
    angular.element("#nav-bar-menu li").removeClass("active");
    $scope.recipeId = $routeParams.recipeId;
    $scope.recipe = {};
    $scope.recipe.Visibility = "public";
    ...
    $scope.yes = function () {
        // delete the record
        $recipeServer.delete($scope.recipeId,  function () {
            $dialog.open("Deleted", "The recipe has been deleted.", function () {
                $location.path("");
                $scope.$apply();
            });
        }, function () {
            $dialog.open("Error", "Error occured while trying to delete recipe.", function () {
                $location.path("/view/" + $scope.recipeId);
                $scope.$apply();
            });
        });
