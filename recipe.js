...
function hideNonMatches(children, searchFor) {
...
function selectPage(pageId, updateContentsFunc) {
    ...
    var callback = function() {
        ...
        if (pageId == "#edit" || pageId == "#add") {
            doAjaxJson("Cmd=GetTags", function(json) {
                setupTags("edit", handleJson(json));
            });
        }
...
function menuTriggerFunc() {
    ...
    if (id == "butLogin") {
        ...
        if (pageId == "#add") {
            doAjaxJson("Cmd=GetTags", function(json) {
                setupTags("add", handleJson(json));
            });
        } else if (pageId == "#find") {
            doAjaxJson("Cmd=GetTags", function(json) {
                setupTags("find", handleJson(json));
            });
...
function handleJson(json) {
    ...
    switch (displayType) {
        case 'ERROR':
            ...
            $('#myModal div.modal-body').empty().append(errMsg);
            $('#myModal div.modal-title').empty().append('Error');
...
//            $.showDialog('error', 'Error');
        ...
        case 'MESSAGE':
            ...
            if ("string" === typeof(msg))
                msg = msg.split("\n").join("<br/>");
            $('#myModal div.modal-body').empty().append(msg);
            $('#myModal div.modal-title').empty().append('Message');
            $('#myModal').modal({show: true});
...
//            $.setDialogButtons($.dialog.buttonTypes.OK_CANCEL, {OK: function() {
//                    loadPageAjax(displayData["OkCmd"]);
//                }});
//            $.showDialog('dialogBlank', 'Message');
        ...
        case 'LIST-RECIPE':
            selectPage("#list-recipe", function() {
                ...
                hideNonMatches(rows, $("#input-search").text());
            });
            break;
        case 'LIST':
...
function doAjaxJson(data, successFunc, errorFunc) {
    ...
    if (errorFunc == undefined) {
        errorFunc = function(error) {
            alert("Error loading ajax return object:\n\n" + error.responseText);
        };
    }
