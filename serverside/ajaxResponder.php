		...
		case 'view':
			...
			if ($recipe == null){
				...
				if ($_SESSION['isadmin'] ||														// we can if we are an admin
					...
					$returnMsg->setMsgType(AjaxMessage::TYPE_ITEM);
					$returnMsg->setURL($rtnUrl);//"view/Hash=" . $Hash);
					$returnMsg->setData($recipe);
				} else {
					$returnMsg->returnError("You are not allowed to view this recipe. It has not been shared.");
				}
			}
			break;
	}
	...
	if ($_SESSION['loggedin'] != true){
		switch ($_POST['Cmd']){
			case 'Login':
				...
				$returnMsg->returnText("response", "Username or Password incorrect or<br/> Account has not been confirmed.");

				$name = mysql_real_escape_string($_POST['user'],getConnection());
				$user = User::findByName($name);
				if ($user != null){
					if (strcmp($user->Confirmation,"") != 0){
						$returnMsg->returnText("response", "Account has not been confirmed. Check your email inbox and your spam folder for the confirmation email.");
					} else {
						...
						if (strcmp($user->Pass,$hash) == 0){
							...
							$returnMsg->setMsgType(AjaxMessage::TYPE_REFRESH);
						}
					}
				}
				break;
			...
			case 'ResetPass':
				...
				if ($user == null){
					...
					mail($email,
						 ...
						 "Your password has been reset as per your request on Recipe Note.\n\n Your new password is: $pass\n\n Log into Recipe note at: http://www.gluonporridge.net/recipetest/ and change your password after logging in with your new password.",
						 "From: RecipeNote@gluonporridge.net");
			...
			case 'Register':
				...
				if ($existingUser != null){
					...
					mail($_POST['Email'],
						 "Recipe Note: Registration Confirmation",
						 "Thank you for registering with Recipe Note. To Complete the registration process click on the link below or copy and paste it into a new browser window:\n\n" .
							"http://www.gluonporridge.net/recipetest/#username=$user->Name&confirm=$user->Confirmation\n\nIf you didn't register with Recipe Note recently please ignore this email.",
						 "From: RecipeNote@gluonporridge.net");
